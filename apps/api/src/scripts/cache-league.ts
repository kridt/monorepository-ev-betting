/**
 * Quick League Cache
 * Usage: npx tsx src/scripts/cache-league.ts <leagueId> <leagueName>
 * Example: npx tsx src/scripts/cache-league.ts 271 "Danish Superliga"
 */
...
async function main() {
  ...
  const LEAGUE_ID = parseInt(args[0]) || 271;
  const LEAGUE_NAME = args[1] || 'Danish Superliga';

  console.log(`🏆 ${LEAGUE_NAME} Quick Cache`);
  console.log('='.repeat(50));

  // Get current season
  console.log('\n📅 Getting current season...');
  const league = await fetchAPI<any>(`/leagues/${LEAGUE_ID}`, { include: 'currentSeason' });
  const seasonId = league?.currentseason?.id;

  if (!seasonId) {
    console.log('  ❌ No current season found for this league');
    return;
  }
  ...
  for (let i = 0; i < teams.length; i++) {
    ...
    await client.execute({
      sql: `INSERT OR REPLACE INTO soccer_teams (id, name, short_code, country_id, league_id, image_path, last_updated)
            VALUES (?, ?, ?, ?, ?, ?, datetime('now'))`,
      args: [team.id, team.name, team.short_code, team.country_id, LEAGUE_ID, team.image_path],
    });
    ...
    for (const entry of squad) {
      ...
      for (const lineup of games) {
        ...
        let opponent = '', opponentId = 0, isHome = false;
        ...
        const gameData = { fixtureId: fixture.id, gameDate: fixture.starting_at?.split('T')[0] || '', opponent, opponentId, isHome, ...stats };
      ...
      const position = entry.position_id === 24 ? 'GK' : entry.position_id === 25 ? 'DEF' : entry.position_id === 26 ? 'MID' : 'FWD';
      ...
      await client.execute({
        ...
        args: [
          ...
          LEAGUE_ID, LEAGUE_NAME, entry.position_id, position,
          player.image_path, player.date_of_birth,
          avg(allGames.map(g => g.shots || 0)), avg(allGames.map(g => g.shotsOnTarget || 0)),
          avg(allGames.map(g => g.goals || 0)), avg(allGames.map(g => g.assists || 0)),
          avg(allGames.map(g => g.passes || 0)), avg(allGames.map(g => g.keyPasses || 0)),
          avg(allGames.map(g => g.tackles || 0)), avg(allGames.map(g => g.interceptions || 0)),
          avg(allGames.map(g => g.clearances || 0)), avg(allGames.map(g => g.blocks || 0)),
          avg(allGames.map(g => g.fouls || 0)), avg(allGames.map(g => g.foulsDrawn || 0)),
          avg(allGames.map(g => g.dribbles || 0)), avg(allGames.map(g => g.duelsWon || 0)),
          avg(allGames.map(g => g.touches || 0)), avg(allGames.map(g => g.minutes || 0)),
          avg(last5.map(g => g.shots || 0)), avg(last5.map(g => g.shotsOnTarget || 0)),
          avg(last5.map(g => g.goals || 0)), avg(last5.map(g => g.assists || 0)),
          avg(last5.map(g => g.passes || 0)), avg(last5.map(g => g.tackles || 0)),
          avg(last10.map(g => g.shots || 0)), avg(last10.map(g => g.shotsOnTarget || 0)),
          avg(last10.map(g => g.goals || 0)), avg(last10.map(g => g.assists || 0)),
          avg(last10.map(g => g.passes || 0)), avg(last10.map(g => g.tackles || 0)),
          avg(homeGames.map(g => g.shots || 0)), avg(homeGames.map(g => g.shotsOnTarget || 0)),
          avg(homeGames.map(g => g.goals || 0)), avg(homeGames.map(g => g.assists || 0)),
          avg(awayGames.map(g => g.shots || 0)), avg(awayGames.map(g => g.shotsOnTarget || 0)),
          avg(awayGames.map(g => g.goals || 0)), avg(awayGames.map(g => g.assists || 0)),
  ...
  await client.execute({
    ...
    args: [LEAGUE_ID, LEAGUE_NAME, null, null, seasonId],
  });

  console.log('\n' + '='.repeat(50));
  console.log(`✅ ${LEAGUE_NAME} Cache Complete!`);
